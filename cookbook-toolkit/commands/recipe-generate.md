...
For any Temporal-correctness decision while writing the workflow and activities (determinism,
signals, child workflows, continue-as-new, cancellation, heartbeats, activity timeout and
retry semantics), consult the canonical Temporal sources rather than relying on memory: the
`temporal:temporal-developer` skill and the `temporal-docs` MCP. See `code-conventions.md`
