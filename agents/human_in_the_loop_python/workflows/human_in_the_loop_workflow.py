...
with workflow.unsafe.imports_passed_through():
    ...
    from activities import bedrock, execute_action, notify_approval_needed
...
class HumanInTheLoopWorkflow:
    ...
    async def run(self, input: WorkflowInput) -> str:
        ...
        workflow.logger.info(
            f"Starting human-in-the-loop workflow for request: {input.user_request}"
        )
        ...
        if proposed_action.risky_action:
            ...
            workflow.logger.info(
                "Action is safe, auto-approving and proceeding with execution"
            )
            result = await workflow.execute_activity(
                execute_action.execute_action,
                proposed_action,
    ...
    async def _request_approval(
        self, proposed_action: ProposedAction, context: str, timeout_seconds: int
    ) -> str:
